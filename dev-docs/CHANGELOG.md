...
**변경 내용:**
- `camera_publisher.py` L1759: `_yaw_w` 추출 오류 수정
  - **버그**: `atan2(M[1][0], M[0][0])` — GfMatrix4d row-vector 규칙에서 M[1][0]=−sin θ 이므로 결과가 −θ (부호 반전)
  - **수정**: `atan2(M[0][1], M[0][0])` — M[0][1]=+sin θ 이므로 올바른 +θ 반환
  - **증상**: 로봇이 fence 방향(북쪽, +Y)을 향할 때 카메라가 반대 방향(후방)을 주시하는 문제. 동·서 방향에서는 우연히 올바른 동작(부호 오류가 상쇄). fence 접근 시에만 작동하는 것처럼 보였던 원인.
