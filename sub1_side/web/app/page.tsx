...
import {
  ...
  FallPayload, WindState, WeaponState, TacticalDetectionPoint,
} from "@/lib/api";
...
export default function Page() {
  ...
  const [tacticalDetections, setTacticalDetections] =
    useState<TacticalDetectionPoint[]>([]);
  ...
  useEffect(() => {
    const pull = async () => {
      try {
        const [st, gps, mission] = await Promise.all([
          getJSON<any>(`/robots/${ROBOT_ID}/state`),
          getJSON<any>(`/robots/${ROBOT_ID}/gps`),
          getJSON<any>("/missions/state"),
        ]);
        setSnap((s) => ({ ...s, ...st, gps: gps.gps }));
        if (mission?.landmarks && Object.keys(mission.landmarks).length) {
          setLandmarks(mission.landmarks);
        }
        if (mission?.patrol_state && Object.keys(mission.patrol_state).length) {
          setPatrolState(mission.patrol_state);
        }
        const intruderItems = Array.isArray(mission?.intruders)
          ? mission.intruders
          : (mission?.intruders?.items ?? []);
        if (intruderItems.length) setIntruders(intruderItems);
      } catch {}
    };
    pull();
  ...
  const onEvent = useCallback((e: C2Event) => {
    if (e.type === "state") {
      ...
      const mapItems = items
        .filter((it: any) => it?.map && it?.camera?.startsWith("tp_"))
        .map((it: any, idx: number) => ({
          id: `${it.camera}:${idx}`,
          x: Number(it.map.x),
          y: Number(it.map.y),
          z: Number(it.map.z ?? 0),
          label: String(it.class_name ?? it.label ?? "unknown"),
          camera: String(it.camera),
          confidence: Number(it.conf ?? it.confidence ?? 0),
          range_m: Number(it.map.range_m ?? 0),
          ground_range_m: Number(it.map.ground_range_m ?? 0),
          risk: String(it.risk ?? "caution"),
          source: String(it.map.source ?? "unknown"),
          tsMs: now,
        }));
      if (mapItems.length) {
        const cameras = new Set(mapItems.map((it) => it.camera));
        setTacticalDetections((prev) => [
          ...prev.filter((it) =>
            now - it.tsMs < 5000 && !cameras.has(it.camera)),
          ...mapItems,
        ].slice(-40));
      }
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      const now = Date.now();
      setTacticalDetections((prev) => prev.filter((it) => now - it.tsMs < 5000));
    }, 1000);
    return () => clearInterval(iv);
  }, []);
  ...
  const cur = snap.odom?.x != null
    ? { x: snap.odom.x, y: snap.odom.y!, z: snap.odom.z, yaw: snap.odom.yaw }
    : null;
