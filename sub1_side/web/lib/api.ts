...
// 빌드타임 env가 있으면 그걸 쓰고, 없으면 Next rewrites(/c2,/robots)를
// 사용해 같은 오리진(:3000)으로 붙는다. 이렇게 해야 localhost/LAN/터널
// 접속이 바뀌어도 MJPEG img src 가 흔들리지 않는다.
...
export function getWsBase(): string {
  if (_STATIC_BASE) return _STATIC_BASE.replace(/^http/, "ws");
  if (typeof window === "undefined") return "";
  const proto = window.location.protocol === "https:" ? "wss" : "ws";
  return `${proto}://${window.location.hostname}:8000`;
}

export const API_BASE = _STATIC_BASE || ""; // SSR 호환용 (fetch 직접 호출 시 fallback)
...
export type TacticalDetectionPoint = {
  id?: string;
  x: number;
  y: number;
  z?: number;
...
export type LandmarksPayload = {
  ...
  home?: Landmark;
  goal?: Landmark;
  tactical_points?: Record<string, Landmark>;
  overhead?: Landmark & { extent?: number };
