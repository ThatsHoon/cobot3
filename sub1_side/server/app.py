...
async def mission_state():
    ...
    if not landmarks:
      try:
          landmarks = json.loads(Path("/tmp/cobot3_landmarks.json").read_text())
          ros.latest["landmarks"] = landmarks
      except Exception:
          landmarks = {}
    return {
...
async def mjpeg(camera: str = "rear"):
    ...
    cam = camera if camera in (
        "rear", "inspect", "overhead", "tp_a", "tp_b", "tp_c", "tp_d",
    ) else "rear"
    async def gen():
        while True:
