    ...
    class _C2Node(Node):
        def __init__(self, br: "RosBridge"):
            ...
            self.get_logger().info(
                "구독: state/gps/odom/leg/rosout/video_front/video_rear/"
                "intruders/patrol_state/landmarks — "
                "업링크: cmd_vel/nav_goal/speaker/fire/mission/inspect/alerts")
        ...
        def _on_video(self, msg, camera: str = "front"):
            ...
            if self.br._yolo is not None and camera == "front":
                ...
                if dets:
                    ...
                    # /detections_text 발행 (DMZ 호환)
                    det_msg = String()
                    det_msg.data = json.dumps({
                        "stamp": ts, "frame_id": "camera_front",
                        "detections": dets})
