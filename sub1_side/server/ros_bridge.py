...
try:
    ...
    from sensor_msgs.msg import NavSatFix, JointState, CompressedImage, Image
...
class RosBridge:
    ...
    def __init__(self):
        ...
        self._video_tp_a:     np.ndarray | None = None  # TP_A 고정 감시카메라
        self._video_tp_b:     np.ndarray | None = None  # TP_B 고정 감시카메라
        self._video_tp_c:     np.ndarray | None = None  # TP_C 고정 감시카메라
        self._video_tp_d:     np.ndarray | None = None  # TP_D 고정 감시카메라
        self._depth_frames: dict[str, np.ndarray] = {}  # TP depth, meters
        self._loop = None
        self._db = None
    ...
    def start(self, loop, db, event_cb, yolo=None):
        ...
        for k, v in env.items():
            log.info("  %s=%s", k, v)
        warns = []
        if env["ROS_DOMAIN_ID"] not in ("129", config.ROS_DOMAIN_ID):
            warns.append(f"ROS_DOMAIN_ID={env['ROS_DOMAIN_ID']} (기대 129)")
    ...
    def _set_depth_frame(self, camera: str, depth_m: np.ndarray):
        with self._video_lock:
            self._depth_frames[camera] = depth_m

    def _get_depth_frame(self, camera: str):
        with self._video_lock:
            arr = self._depth_frames.get(camera)
...
if RCLPY_OK:
    class _C2Node(Node):
        def __init__(self, br: "RosBridge"):
            ...
            self.create_subscription(CompressedImage, T["video_tp_a"],
                                     lambda m: self._on_video(m, "tp_a"), sensor_qos)
            self.create_subscription(CompressedImage, T["video_tp_b"],
                                     lambda m: self._on_video(m, "tp_b"), sensor_qos)
            self.create_subscription(CompressedImage, T["video_tp_c"],
                                     lambda m: self._on_video(m, "tp_c"), sensor_qos)
            self.create_subscription(CompressedImage, T["video_tp_d"],
                                     lambda m: self._on_video(m, "tp_d"), sensor_qos)
            self.create_subscription(Image, T["depth_tp_a"],
                                     lambda m: self._on_depth(m, "tp_a"), sensor_qos)
            self.create_subscription(Image, T["depth_tp_b"],
                                     lambda m: self._on_depth(m, "tp_b"), sensor_qos)
            self.create_subscription(Image, T["depth_tp_c"],
                                     lambda m: self._on_depth(m, "tp_c"), sensor_qos)
            self.create_subscription(Image, T["depth_tp_d"],
                                     lambda m: self._on_depth(m, "tp_d"), sensor_qos)
        ...
        def _health(self):
            ...
            pubs = {
                ...
                "video_tp_a":    self.count_publishers(T["video_tp_a"]),
                "video_tp_b":    self.count_publishers(T["video_tp_b"]),
                "video_tp_c":    self.count_publishers(T["video_tp_c"]),
                "video_tp_d":    self.count_publishers(T["video_tp_d"]),
        ...
        def _decode_depth(self, msg):
            ...
            if enc in ("32fc1", "float32"):
                arr = np.frombuffer(raw, dtype=np.float32).reshape(h, -1)
                arr = arr[:, :w]
            elif enc in ("16uc1", "mono16"):
                arr = np.frombuffer(raw, dtype=np.uint16).reshape(h, -1)
                arr = arr[:, :w].astype(np.float32) / 1000.0
            elif enc in ("8uc1", "mono8"):
                arr = np.frombuffer(raw, dtype=np.uint8).reshape(h, -1)
                arr = arr[:, :w].astype(np.float32)
            else:
                # Isaac depth is normally 32FC1. Unknown encodings are ignored
                # rather than poisoning target positions with byte-shaped data.
                return None
        ...
        def _sample_depth(self, camera: str, det: dict,
            ...
            dh, dw = depth.shape[:2]
            x, y, w, h = det["bbox"]
            # For upright classes, bbox center/bottom often contains ground,
            # legs, or fence pixels. Sample the upper torso region for a more
            # stable target depth, then project that same pixel ray to map.
            ...
            if label in ("person", "soldier"):
                ...
                rw = max(4.0, w * 0.25)
                rh = max(4.0, h * 0.25)
            sx = dw / max(float(iw), 1.0)
            sy = dh / max(float(ih), 1.0)
            x0 = max(0, int((cx - rw) * sx))
            x1 = min(dw, int((cx + rw) * sx) + 1)
            y0 = max(0, int((cy - rh) * sy))
            y1 = min(dh, int((cy + rh) * sy) + 1)
            if x1 <= x0 or y1 <= y0:
                return None
        ...
        @staticmethod
        def _norm3(v):
            n = math.sqrt(float(v[0]) ** 2 + float(v[1]) ** 2
                          + float(v[2]) ** 2)
            if n < 1e-6:
                return (0.0, 1.0, 0.0)
        ...
        def _project_detection_to_map(self, camera: str, det: dict,
            ...
            if not lm.get("tactical_points"):
                try:
                    ...
                    pass
            tps = lm.get("tactical_points") or lm.get("tp") or {}
            tp_name = camera.upper()
            tp = tps.get(tp_name)
            if not tp:
                return None
            ih, iw = image_shape[:2]
            ...
            if depth_sample is None:
                ...
                sample_u = float(depth_sample["u"])
                sample_v = float(depth_sample["v"])
            ...
            if rn > 1e-6:
                rx, ry = rx / rn, ry / rn
            fx_px = (float(iw) / config.TACTICAL_CAMERA_APERTURE
                     * config.TACTICAL_CAMERA_FOCAL)
            fy_px = fx_px
            x_norm = (sample_u - iw * 0.5) / max(fx_px, 1.0)
            y_norm = (sample_v - ih * 0.5) / max(fy_px, 1.0)
            ux = ry * fwd[2]
            uy = -rx * fwd[2]
            ...
            # Isaac depth is treated like optical-axis depth. Project the same
            # sampled pixel ray instead of only shifting x in the ground map.
            world_x = float(tp["x"]) + depth_m * (fwd[0] + rx * x_norm - ux * y_norm)
            world_y = float(tp["y"]) + depth_m * (fwd[1] + ry * x_norm - uy * y_norm)
            world_z = cam_z + depth_m * (fwd[2] - uz * y_norm)
            ground_range_m = math.hypot(
                world_x - float(tp["x"]), world_y - float(tp["y"]))
            slant_range_m = math.sqrt(
                (world_x - float(tp["x"])) ** 2
                + (world_y - float(tp["y"])) ** 2
                + (world_z - cam_z) ** 2)

            # /robot/odom and Tactical Map use local coordinates from the
            # robot start position. Tactical camera prims are in Isaac world.
            home = lm.get("home") or {}
            ...
            return {
                ...
                "depth_camera": camera,
                "depth_topic": config.TOPICS.get(f"depth_{camera}", ""),
                "source": source,
            }

        def _on_video(self, msg, camera: str = "rear"):
            # YOLO는 고정 감시카메라에서만 돌린다. 결과는 영상 bbox가 아니라
            # Tactical Map 점으로만 사용한다.
            key = f"video_{camera}"
            self._rx[key] = self._rx.get(key, 0) + 1
            if self._rx[key] == 1:
                self.get_logger().info(
            ...
            if bgr is None:
                return
            yolo_cameras = {"tp_a", "tp_b", "tp_c", "tp_d"}
            if self.br._yolo is not None and camera in yolo_cameras:
                ...
                if dets:
                    ...
                    for d in dets:
                        d["camera"] = camera
                        d["frame_id"] = frame_id
                        d["risk"] = self._risk_for(d.get("class_name", ""))
                        map_pos = self._project_detection_to_map(
                            camera, d, bgr.shape)
