...
class DBWriter:
    ...
    async def start(self):
        if str(config.DB_URL).strip().lower() in ("", "none", "off", "disabled"):
            self._disabled = True
            log.info("DBWriter disabled (COBOT3_DB_URL=%s)", config.DB_URL)
            return
        try:
            self._pool = await asyncpg.create_pool(
                config.DB_URL, min_size=1, max_size=4)
        except Exception as e:
            self._disabled = True
            log.warning("DBWriter disabled: cannot connect to %s (%s)",
                        config.DB_URL, e)
            return
