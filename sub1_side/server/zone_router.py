...
class ZoneRouter:
    def __init__(
        ...
        if routing_edges is not None:
            self._graph = self._build_from_edges(routing_edges)
        else:
            self._graph = self._build(max_edge_m)

    def _build_from_edges(
        self, routing_edges: list[dict]
