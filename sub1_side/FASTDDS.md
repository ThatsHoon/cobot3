...
> 시뮬 PC(Isaac)의 ROS2 토픽을 안정적으로 수신·발행하기 위한 설정을 다룬다.
> HTTP `/ingest` 경로는 제거됨 — ROS2 정공 단일 경로(업링크·다운링크).
...
| 증상 | 원인 | 조치 |
|---|---|---|
| **C2 토픽 0 인데 Isaac 는 ROS2 발행 중**(로그 `OG 텔레메트리 발행`) | ★1순위★ **Isaac 가 `fastdds_no_shm.xml`(initialPeers 없음=멀티캐스트 전용)로 기동** → 크로스호스트 멀티캐스트 차단 시 C2 가 영영 못 봄 | Isaac PC: `FASTRTPS_DEFAULT_PROFILES_FILE` 확인 → `~/.config/cobot3/fastdds_main.xml`(initialPeers→C2) 여야. 수동: `FASTRTPS_DEFAULT_PROFILES_FILE="$(cobot3_fastdds_profile main)"` 후 재기동 |
| `ros2 topic list` 에 시뮬 토픽 없음 | 멀티캐스트 차단 또는 IP 오설정 | 위 항목 먼저, §3 initialPeers·interfaceWhiteList(양 PC site.env=실 NIC), 방화벽 §5 |
| 토픽은 보이나 영상만 0 | QoS 불일치 또는 멀티-NIC | §2 QoS 확인, §3 interfaceWhiteList IP 확인 |
| cmd_vel 발행했는데 Spot 미이동 | SpotController 미기동 | Isaac 로그 `[spot_ctrl] ready` 확인, `GP_SPOT_CONTROL` 확인 |
| 영상 끊김/지연 | 버퍼 부족 | §3 버퍼·§4 OS 버퍼, video_degrade 동작 확인 |

ros_bridge 의 5초 주기 `HEALTH` 로그(`rx=… publishers=…`)와
