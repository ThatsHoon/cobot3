...
parser.add_argument("--no-deer", action="store_true", help="Disable the deer scenario.")
parser.add_argument("--deer-count", type=int, default=2, help="Number of deer to spawn.")
parser.add_argument("--deer-speed", type=float, default=0.6, help="Deer movement speed in meters per second.")
parser.add_argument("--deer-seed", type=int, default=77, help="Seed for deterministic deer spawn positions.")
parser.add_argument(
    "--deer-usd",
    type=str,
    default=str(PROJECT_ROOT / "deer/Stylized_Deer.usdz"),
    help="Path to the deer USD/USDZ asset.",
)
parser.add_argument(
    "--deer-yaw-deg",
    type=float,
    default=0.0,
    help="Yaw rotation applied to the deer visual in degrees.",
)
...
parser.add_argument(
    "--replicator-deer",
    action="store_true",
    help="Generate a deer (animal) Replicator dataset, then exit.",
)
parser.add_argument(
    "--dataset-frames",
    type=int,
    default=60,
    help="Number of synthetic frames to capture when --replicator-dataset or --replicator-deer is enabled.",
)
parser.add_argument(
    "--dataset-output-dir",
    type=str,
...
FRONT_CAMERA_TRANSLATION = (0.0, 0.95, 0.64)
...
# USD camera local -Z looks forward. This quaternion points it along robot +Y
# (toward the fence / north direction) while keeping robot +Z as image up.
FRONT_CAMERA_ORIENTATION_IJKR = (0.7071, 0.0, 0.0, 0.7071)
...
def _create_deer_visual(stage, path: str, usd_path: str, yaw_deg: float) -> dict:
    root = UsdGeom.Xform.Define(stage, Sdf.Path(path))
    root_prim = root.GetPrim()
    xformable = UsdGeom.Xformable(root_prim)
    translate_op = xformable.AddTranslateOp(UsdGeom.XformOp.PrecisionDouble)
    xformable.AddRotateXYZOp(UsdGeom.XformOp.PrecisionDouble).Set(Gf.Vec3d(0.0, 0.0, -90.0 + float(yaw_deg)))
    xformable.AddScaleOp(UsdGeom.XformOp.PrecisionDouble).Set(Gf.Vec3d(0.3, 0.3, 0.3))

    resolved = str(Path(usd_path).expanduser())
    if Path(resolved).exists():
        try:
            model_prim = add_reference_to_stage(usd_path=resolved, prim_path=f"{path}/Model")
            if model_prim is not None and model_prim.IsValid():
                _label_prim_tree(model_prim, "animal")
                print(f"Deer asset loaded: {resolved}")
                return {"prim": root_prim, "translate_op": translate_op, "visual_type": "deer-usd"}
        except Exception as exc:
            carb.log_warn(f"Failed to load deer asset '{resolved}': {exc}")
    else:
        carb.log_warn(f"Deer asset not found: {resolved}. Using primitive fallback.")

    _apply_class_label(root_prim, "animal")
    brown = (0.45, 0.28, 0.10)
    dark = (0.25, 0.15, 0.05)
    _add_visual_cube(stage, f"{path}/Body", (0.0, 0.0, 0.55), (0.35, 0.65, 0.30), brown, "animal")
    _add_visual_sphere(stage, f"{path}/Head", (0.0, 0.38, 0.72), 0.13, brown, "animal")
    _add_visual_cube(stage, f"{path}/Neck", (0.0, 0.27, 0.65), (0.10, 0.18, 0.20), brown, "animal")
    _add_visual_cube(stage, f"{path}/FL", (-0.10, 0.25, 0.22), (0.07, 0.07, 0.42), dark, "animal")
    _add_visual_cube(stage, f"{path}/FR", (0.10, 0.25, 0.22), (0.07, 0.07, 0.42), dark, "animal")
    _add_visual_cube(stage, f"{path}/RL", (-0.10, -0.25, 0.22), (0.07, 0.07, 0.42), dark, "animal")
    _add_visual_cube(stage, f"{path}/RR", (0.10, -0.25, 0.22), (0.07, 0.07, 0.42), dark, "animal")
    return {"prim": root_prim, "translate_op": translate_op, "visual_type": "deer-primitive"}
...
class DeerScenario:
    def __init__(
        ...
        stage.DefinePrim("/World/Deer", "Xform")
        for index in range(max(0, int(count))):
            path = f"/World/Deer/Deer_{index}"
            handles = _create_deer_visual(stage, path, usd_path, yaw_deg)
            state = {
                "path": path,
            ...
            self._respawn(state)
            self._deer.append(state)

        if self._deer:
            print(f"Deer scenario enabled: count={len(self._deer)}, speed={self._speed:.2f} m/s")

    def _respawn(self, state: dict) -> None:
    ...
    def _set_pose(self, state: dict) -> None:
        position = state["position"]
        ground_z = _sample_height(self._x_values, self._y_values, self._heights, position[0], position[1])
        bob = 0.03 * np.sin(state["time"] * 4.0 + state["phase"]) if not state["arrived"] else 0.0
        state["translate_op"].Set(Gf.Vec3d(float(position[0]), float(position[1]), float(ground_z + bob + 0.45)))

    def update(self, step_size: float) -> None:
        for state in self._deer:
            state["time"] += float(step_size)
            if state["arrived"]:
                state["hold_time"] += float(step_size)
                if state["hold_time"] > 6.0:
                    self._respawn(state)
                else:
                    self._set_pose(state)
                continue
    ...
    def get_positions(self) -> list[tuple[float, float]]:
        return [(float(s["position"][0]), float(s["position"][1])) for s in self._deer]

    def randomize_for_dataset(self) -> list[tuple[float, float]]:
        for state in self._deer:
            self._respawn(state)
...
class Anymal_runner(object):
    def __init__(
        ...
        _deer_usd = deer_usd or str(PROJECT_ROOT / "deer/Stylized_Deer.usdz")
        self._deer_scenario = (
            DeerScenario(
                self._stage,
                terrain_x_values,
        ...
        # 카메라 orient op 참조 저장 (철책 방향 동적 회전용)
        self._camera_orient_op = None
        self._camera_parent_path = ""
        if enable_ros2_sensors:
            cam_path = self._ros_sensor_handles.get("camera_path", "")
            if cam_path:
                cam_prim = self._stage.GetPrimAtPath(cam_path)
                if cam_prim.IsValid():
                    ops = UsdGeom.Xformable(cam_prim).GetOrderedXformOps()
                    if len(ops) >= 2:
                        self._camera_orient_op = ops[1]
                        self._camera_parent_path = cam_path.rsplit("/", 1)[0]
    ...
    def _handle_inspection_camera_command(self, data: str) -> None:
        ...
        if action in ("look_at", "track", "select"):
            ...
            if "focal_length" in payload:
                self._inspection_camera_focal_length = float(np.clip(
                    float(payload["focal_length"]),
                    INSPECTION_CAMERA_MIN_FOCAL_LENGTH,
                    INSPECTION_CAMERA_MAX_FOCAL_LENGTH,
                ))
            print(f"Inspection camera tracking target: x={x:.2f}, y={y:.2f}, z={z:.2f}, focal={self._inspection_camera_focal_length:.0f}mm")
    ...
    def _update_camera_fence_orientation(self) -> None:
        """카메라 부모 prim의 월드 회전을 XformCache로 직접 읽어
        q_local = q_parent_world_inv * q_fence_target 를 매 프레임 설정.
        로봇의 yaw/roll/pitch 모두 상쇄되어 항상 세계 +Y(철책)를 향함."""
        if self._camera_orient_op is None or not self._camera_parent_path:
            return
        parent_prim = self._stage.GetPrimAtPath(self._camera_parent_path)
        if not parent_prim.IsValid():
            return

        cache = UsdGeom.XformCache()
        mat = cache.GetLocalToWorldTransform(parent_prim)
        q_gf = mat.ExtractRotationQuat()
        pw = float(q_gf.GetReal())
        pimg = q_gf.GetImaginary()
        px, py, pz = float(pimg[0]), float(pimg[1]), float(pimg[2])
        ...
        self._camera_orient_op.Set(Gf.Quatf(float(rw), Gf.Vec3f(float(rx), float(ry), float(rz))))
    ...
    def _update_inspection_camera_pose(self) -> None:
        ...
        if self._inspection_camera_target is not None:
            # 지도 클릭으로 선택된 타겟을 바라봄
            w, x, y, z = _camera_look_at_quat(camera_position, self._inspection_camera_target)
        else:
            # 기본: 철책을 바라봄 (수동 pan/tilt 적용)
            dx = 0.0
            dy = self._fence_y - camera_position[1]
            pan = self._inspection_camera_manual_pan
            new_dx = dx * math.cos(pan) - dy * math.sin(pan)
            new_dy = dx * math.sin(pan) + dy * math.cos(pan)
            horiz_dist = math.hypot(new_dx, new_dy)
            tilt_z = math.tan(self._inspection_camera_manual_tilt) * max(horiz_dist, 0.1)
            fence_target = np.array([
                camera_position[0] + new_dx,
                camera_position[1] + new_dy,
                0.8 + tilt_z,
            ], dtype=float)
            w, x, y, z = _camera_look_at_quat(camera_position, fence_target)
    ...
    def on_physics_step(self, step_size) -> None:
        ...
        if self.first_step:
            ...
            if self._deer_scenario is not None:
                self._deer_scenario.update(0.0)
            self._update_lidar_follow_pose()
            self._update_camera_fence_orientation()
            ...
            if self._deer_scenario is not None:
                self._deer_scenario.update(step_size)
            self._update_lidar_follow_pose()
            self._update_camera_fence_orientation()
            self._update_inspection_camera_pose()
    ...
    def generate_deer_dataset(
        ...
        if self._deer_scenario is None:
            carb.log_warn("Deer dataset requested but deer scenario is disabled (--no-deer).")
            return
        ...
        frame_count = max(1, int(frame_count))
        print(
            f"Deer Replicator dataset: frames={frame_count}, "
            f"output={output_path}, resolution={int(image_width)}x{int(image_height)}"
        )

        for frame_idx in range(frame_count):
            deer_positions = self._deer_scenario.randomize_for_dataset()
            simulation_app.update()

            if deer_positions:
                target_xy = deer_positions[int(rng.integers(0, len(deer_positions)))]
                target_z = float(
                    _sample_height(
                        self._terrain_x_values, self._terrain_y_values, self._terrain_heights,
                        target_xy[0], target_xy[1],
                    ) + 0.6
                )
                target = (float(target_xy[0]), float(target_xy[1]), target_z)
            else:
                target = (0.0, self._fence_y + 1.5, 0.6)

            profile = rng.choice(["close", "medium", "wide"], p=[0.45, 0.35, 0.20])
            if profile == "close":
                cam_x = target[0] + float(rng.uniform(-4.0, 4.0))
                cam_y = self._fence_y - float(rng.uniform(1.0, 5.0))
                cam_z = float(rng.uniform(0.6, 1.3))
                jitter = (0.6, 0.4, 0.3)
            elif profile == "medium":
                cam_x = target[0] + float(rng.uniform(-8.0, 8.0))
                cam_y = self._fence_y - float(rng.uniform(4.0, 10.0))
                cam_z = float(rng.uniform(0.5, 1.5))
                jitter = (1.2, 0.8, 0.4)
            else:
                cam_x = target[0] + float(rng.uniform(-15.0, 15.0))
                cam_y = self._fence_y - float(rng.uniform(8.0, 18.0))
                cam_z = float(rng.uniform(0.4, 1.8))
                jitter = (2.5, 1.5, 0.5)

            cam_x = float(np.clip(cam_x, -self._terrain_size * 0.38, self._terrain_size * 0.38))
            cam_y = float(np.clip(cam_y, -self._terrain_size * 0.46, self._terrain_size * 0.46))
            look_at = (
                target[0] + float(rng.uniform(-jitter[0], jitter[0])),
                target[1] + float(rng.uniform(-jitter[1], jitter[1])),
                target[2] + float(rng.uniform(-0.1, jitter[2])),
            )

            with camera:
                rep.modify.pose(position=(cam_x, cam_y, cam_z), look_at=look_at)

            simulation_app.update()
            rep.orchestrator.step(rt_subframes=0)
            simulation_app.update()

            if (frame_idx + 1) % 10 == 0 or frame_idx == frame_count - 1:
                print(f"Deer dataset frame {frame_idx + 1}/{frame_count} (profile={profile})")

        try:
            rep.orchestrator.wait_until_complete()
        except Exception:
            pass
        print(f"Deer Replicator dataset written to: {output_path}")
...
def main():
    ...
    runner = Anymal_runner(
        ...
        enable_deer=not args.no_deer,
        deer_count=args.deer_count,
        deer_speed=args.deer_speed,
        deer_seed=args.deer_seed,
        deer_usd=args.deer_usd,
        deer_yaw_deg=args.deer_yaw_deg,
    )
    simulation_app.update()
    runner._world.reset()
    simulation_app.update()
    try:
        ...
        elif args.replicator_deer:
            deer_output = args.dataset_output_dir.replace("person_preview_replicator", "deer_replicator")
            runner.generate_deer_dataset(
                output_dir=deer_output,
                frame_count=args.dataset_frames,
                image_width=args.dataset_width,
                image_height=args.dataset_height,
                seed=args.dataset_seed,
