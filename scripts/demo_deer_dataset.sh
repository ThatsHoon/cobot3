...
DEER_FRAMES="${DEER_FRAMES:-500}"
DEER_COUNT="${DEER_COUNT:-4}"
DATASET_WIDTH="${DATASET_WIDTH:-640}"
DATASET_HEIGHT="${DATASET_HEIGHT:-480}"
DEER_OUTPUT_DIR="${DEER_OUTPUT_DIR:-${PROJECT_ROOT}/datasets/deer_replicator}"
...
echo "Generating deer dataset: ${DEER_FRAMES} frames, ${DEER_COUNT} deer, ${DATASET_WIDTH}x${DATASET_HEIGHT}"
echo "Output: ${DEER_OUTPUT_DIR}"
...
exec "${PROJECT_ROOT}/scripts/run_anymal_gp.sh" \
  ...
  --no-ros2-cmd-vel \
  --no-ros2-odom \
  --deer-count "${DEER_COUNT}" \
  --replicator-deer \
  --dataset-frames "${DEER_FRAMES}" \
  --dataset-width "${DATASET_WIDTH}" \
  --dataset-height "${DATASET_HEIGHT}" \
  --dataset-output-dir "${DEER_OUTPUT_DIR}" \
  "$@"
