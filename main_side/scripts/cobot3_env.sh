...
_cobot3_isaac_up() {
    ...
    if [ "$mode" = mcp ]; then
        ...
        ( cd "$MAIN" && setsid env \
            ...
            ROS_DOMAIN_ID=${ROS_DOMAIN_ID:-129} ROS_LOCALHOST_ONLY=0 \
    ...
    case "$role" in
      MAIN)
        ...
        ( source /opt/ros/humble/setup.bash 2>/dev/null \
            && export ROS_DOMAIN_ID=${ROS_DOMAIN_ID:-129} RMW_IMPLEMENTATION=rmw_fastrtps_cpp \
            && export FASTRTPS_DEFAULT_PROFILES_FILE=$MAIN/fastdds_no_shm.xml \
        ...
        ;;
      C2)
        # ROS2 정공: ros_bridge 가 Isaac 실토픽 직접 구독(domain129, FastDDS).
