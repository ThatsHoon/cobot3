...
#
# 사양: HOME=(212.8, 890.53), GOAL=(287.59, 1129.728), arrive=±10m.
# AABB = HOME↔GOAL 사각형 + 100m 패딩.
#   xmin=112.8, xmax=387.59, ymin=790.53, ymax=1229.728, res=0.5 m/px
#   ≈ 550×878 px ≈ 0.48MB PGM (Nav2 map_server 4MB 한계 안전)
#
...
exec "$PYTHON_SH" bake_gp_static_map.py \
    --xmin 112.8 --xmax 387.59 \
    --ymin 790.53 --ymax 1229.728 \
    --res 0.5 \
    --no-clamp \
    "$@"
